...
if __name__ == "__main__":
    ...
    translation_history = create_accuracy_graph(env, "./saves/8")

    tl_history = []

    for tl in translation_history:
        ntl = {}
        for k, v in tl.items():
            # find most common translation in v
            counts = defaultdict(int)
            for c in v:
                counts[c] += 1
            if len(counts) == 0:
            ...
            max_count_key = [k for k, v in counts.items() if v == max_count]
            if max_count > 2:
                ntl[k] = max_count_key[0]
            else:
                ntl[k] = "N/A"
        tl_history.append(ntl)

    for i, tl in enumerate(tl_history):
        print(f"Episode {i*1000}: {tl}")

    agent = SpeakerListenerPPO(obs_size=OBS_SIZE, vocab_size=VOCAB_SIZE)
    agent.load_from("./saves/8/speaker_listener.pth")
    ...
    # # # else:
    # # agent.load_from("checkpoint9000.pth")
