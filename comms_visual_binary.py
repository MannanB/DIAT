...
OBS_SIZE = 2 # binary
VOCAB_SIZE = 4    # Size of "language" for the speaker
MAX_TURNS = 3     # Max # of turns (also limits seq lenght)
...
class SpeakerListenerModel(nn.Module):
    def __init__(self, obs_size, vocab_size, shapes_size):
        ...
        self.listener_net = ListenerNet(vocab_size, shapes_size)
...
class SpeakerListenerPPO:
    def __init__(self, obs_size, vocab_size, shapes_size, gamma=GAMMA, eps_clip=EPS_CLIP, lr=LR,
        ...
        self.policy = SpeakerListenerModel(obs_size, vocab_size, shapes_size)
        self.old_policy = SpeakerListenerModel(obs_size, vocab_size, shapes_size)
    ...
    def select_action(self, obs_shape, comm_tokens):
        ...
        """
        # obs_shape is [1, 9] (1, obs_seq_len)
...
class VisualCommEnv:
    ...
    def reset(self, current_shape=None):
        ...
        self.current_obs = torch.tensor(self.current_obs).view(-1)
    ...
    def step(self, speaker_action, listener_action):
        ...
        if listener_action == self.current_shape:
...
def test(env, agent, n_episodes=10):
    ...
    for ep in range(n_episodes):
        ...
        while not done:
            (spk_a, _, _, lis_a, _, _) = agent.select_action(obs_tokens, comm_tokens)
            next_obs, reward, done, _ = env.step(spk_a.item(), lis_a.item())
            ep_reward += reward
...
def get_translations(env, agent, n_episodes=10):
    ...
    for ep in range(n_episodes):
        for shape in range(len(shapes)):
            ...
            if shapes_map[env.current_shape] not in translations:
                translations[shapes_map[env.current_shape]] = []
            if env.current_shape == lis_a:
                translations[shapes_map[env.current_shape]].append(comm_str)
                num_correct += 1
    print(f"Accuracy: {num_correct}/{n_episodes*len(shapes)}")
    return translations
...
def TNSEProjection(agent):
    ...
    for i in range(VOCAB_SIZE):
        ...
        embedding = agent.policy.listener_net.embed(comm_tokens)
...
if __name__ == "__main__":
    ...
    agent = SpeakerListenerPPO(obs_size=OBS_SIZE, vocab_size=VOCAB_SIZE, shapes_size=len(shapes))
    ...
    agent.load_from("./saves/visual1/checkpoint8000.pth")

    # Test
    test(env, agent, n_episodes=100)
    translations = get_translations(env, agent, n_episodes=10)
    print(translations)
